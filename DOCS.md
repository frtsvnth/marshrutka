...
- `visible` — показывать ли поле в форме (вместо удаления)
- `supports_default` — может ли поле иметь значение по умолчанию
- `deprecated` — поле устарело (визуально затемняется)
...
- badge «★ Primary»
- в publish block показывается, какой artifact будет опубликован по умолчанию
...
Dashboard показывает все проекты карточками. В каждой карточке:
- название и описание проекта
- queue summary (total, queued, draft, failed)
- schedule summary (активные расписания)
- последний запуск
- primary artifact info
- быстрые действия: открыть проект, очередь, быстрый запуск, запустить следующий
...
Story-to-video (`http://141.136.44.9:8001`) и Ежу понятно (`http://141.136.44.9:8000`).
Оба проекта обновлены в версии 0.5:
...
1. **Schedule** — удалено поле `input`, schedules теперь queue-driven
2. **Run** — добавлено поле `queue_item_id`
3. **Project** — добавлены поля `defaults` и `primary_artifact`
4. **Project page** — перестроен приоритет блоков (очередь на первом месте)
5. **Главная страница** — dashboard вместо списка проектов
6. **Edit project page** — добавлены primary artifact и defaults
7. **Run detail page** — primary artifact отображается и выделяется
...
- **Агент** — AI-агент не был затронут в этой версии. Инструменты агента работают с прежними API.
