...
│   ├── harness.py       # Tool-calling loop, LLM calls, SSE event stream
│   ├── memory.py        # MemoryManager, SessionMemory, ProfileMemory, system prompt
│   ├── models.py        # Pydantic models: SSEEvent, ToolDefinition, ChatRequest
│   ├── router.py        # FastAPI routes: /api/agent/chat, /context, /memory, /history
│   ├── session.py       # session_id validation utility
│   ├── tools.py         # Marshrutka tools + tool definitions for OpenAI function calling
...
│   └── agent-chat.css   # Chat-specific styles
...
AI-агент (Маршал) встроен в правую панель интерфейса. Умеет:
- писать и редактировать тексты с проверкой длины (count_words)
- получать список проектов и их статусы
- просматривать последние запуски
- запускать пайплайны проектов
- создавать и просматривать расписания
- искать информацию в интернете (search_web)
- искать YouTube видео (search_youtube)
- выполнять multi-step reasoning с вызовом нескольких инструментов подряд
...
| Файл | Назначение |
|---|---|
| `router.py` | FastAPI маршруты: POST /chat (SSE), GET /context, POST /memory, DELETE /history |
| `harness.py` | Основной loop: вызов LLM с tools → выполнение tool → повтор (max 5 шагов) |
| `memory.py` | MemoryManager (profile + sessions + project_notes), system prompt builder |
| `models.py` | Pydantic: SSEEvent, ChatRequest (+ message length validation), ToolDefinition |
| `session.py` | Валидация session_id (regex `^[a-zA-Z0-9_-]{8,64}$`) |
| `tools.py` | Marshrutka tools + TOOL_DEFINITIONS для OpenAI function calling |
| `web_tools.py` | search_web, search_youtube через SerpApi |

**Tool calling (native OpenAI):**

Агент использует нативный `tools` параметр OpenAI API (function calling). Модель вызывает инструменты как структурированные JSON, без regex-парсинга из текста. Tool loop:

1. call_llm(messages, tools=TOOL_DEFS)
2. Если model вернула tool_calls → выполнить каждый, добавить результат в messages
3. Повторить, пока модель не вернёт обычный текст (final)
4. Отдать final content через SSE event message_done
...
|---|---|---|
| `session` | `{"session_id": "abc123"}` | ID сессии (первое событие) |
| `message_start` | `{}` | Начало генерации ответа |
| `token` | `{"content": "текст"}` | Токен стриминга (зарезервировано) |
| `tool_start` | `{"tool_name": "...", "arguments": {...}}` | Агент вызывает инструмент |
| `tool_result` | `{"tool_name": "...", "result_summary": "...", "duration_ms": 123}` | Результат инструмента |
| `message_done` | `{"content": "..."}` | Финальный полный ответ ассистента |
| `error` | `{"message": "..."}` | Ошибка (без traceback) |
| `done` | `{}` | Завершение стрима |

### 12.3. Инструменты

| Tool | Описание | Source |
|---|---|---|
| `count_words` | Подсчёт слов, символов, предложений | `tools.py` |
| `get_projects` | Список проектов | `tools.py` |
| `get_recent_runs` | Последние запуски (с фильтром по project_id) | `tools.py` |
| `run_pipeline` | Запуск пайплайна проекта | `tools.py` |
| `create_schedule` | Создание cron-расписания | `tools.py` |
| `get_schedules` | Список расписаний | `tools.py` |
| `search_web` | Поиск в интернете через SerpApi | `web_tools.py` |
| `search_youtube` | Поиск YouTube видео (site:youtube.com/watch) | `web_tools.py` |

### 12.4. Память

**User profile** (`data/agent/user_profile.json`): глобальный, хранит имя, preferences, notes.

**Session memory** (`data/agent/sessions/<session_id>.json`): per-session chat history, последние 20 сообщений в окне, последние 10 отдаются модели.

**Project notes** (`data/agent/project_notes.json`): глобальные заметки о проектах.

Cache: in-memory LRU на 100 сессий (OrderedDict).

### 12.5. API endpoints

| Метод | Путь | Описание |
|---|---|---|
| `POST` | `/api/agent/chat` | Отправить сообщение агенту. Response: SSE stream |
| `GET` | `/api/agent/context?session_id=` | Получить контекст (профиль, проекты, history count) |
| `POST` | `/api/agent/memory` | Обновить user profile (`key` + `value`) |
| `DELETE` | `/api/agent/history?session_id=` | Очистить историю сессии + сбросить cache |
...
```
- `session_id` опционален; если пустой, сервер генерирует новый
- `message` — строка до 8000 символов
...
- **Поле ввода** с поддержкой Enter (Shift+Enter для новой строки)
- **Send-кнопка** ➤
- **Кнопка «📋 Копировать диалог»** — копирует readable transcript с ролями
- **Кнопка «🔍 Debug trace»** — копирует structured JSON (session_id, messages, events)
- **Tool events** — отображаются в виде строк с иконками (⚙/🌐/▶) и summary
- **Защита от двойной отправки** — input disabled во время active запроса
- **Auto-scroll** к новым сообщениям
- **Ошибки** — отображаются в панели красным, без прерывания работы

При быстрых подсказках (quick chips) в начале сессии:
- 📋 Список проектов
- ✍️ Текст на 160 слов
- 📊 Последние запуски
...
# Web search (опционально — поиск работает только при наличии ключа)
...
- **AI-панель** — работает. Агент умеет работать с проектами, текстами и интернет-поиском. Финальный ответ не стримится token-by-token (отдаётся целиком).
