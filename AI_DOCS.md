> **Документ описывает прежнюю версию маршрутки** — ту, что была системой
> управления с очередью, планировщиком и встроенным агентом. Эта версия
> заменена: управление переехало в Hermes, маршрутка стала интерфейсом.
> Актуальное — в [README.md](./README.md). Файл оставлен как справка по
> удалённому коду.

# AI-агент Маршал — документация

> Версия: 2.4 | Последнее обновление: 2026-06-11
...
- **dynamic capability discovery**: поиск новых возможностей в приложении (routes, publish bindings, UI actions), когда пользователь просит операционное действие, отсутствующее среди заранее описанных tools
- **навигация по страницам**: после выполнения действия (запуск пайплайна, редактирование файла, создание расписания) может перенаправить браузер на нужную страницу или обновить текущую
...
├── capabilities.py   # CapabilityRegistry, OperationalIntentAnalyzer, discovery/invoke tools
...
├── capabilities.json      # Кэш capability registry (обнаруженные actions, routes, UI actions)
...
### 4.8. Событие `navigate`
```
event: navigate
data: {"type": "navigate", "url": "/projects/story-to-video", "reload": false}
```
Перенаправление браузера на другую страницу. Фронтенд выполняет `window.location.href = url`. Если `reload: true` — делает `window.location.reload()`.
Отправляется ПОСЛЕ `message_done`, чтобы ответ ассистента успел отрендериться.
...
4. `navigate` — (опционально) перенаправление на другую страницу
...
| Tool | Описание |
|---|---|
| `git_status` | Показать состояние git-репозитория: изменённые, добавленные, неотслеживаемые файлы, ветка |
| `git_commit_push` | Закоммитить и запушить изменения в GitHub. Двухшаговый паттерн: preview → confirm. Сначала вызывается без `confirm=True` для предпросмотра, после подтверждения пользователя — с `confirm=True`. |

### 5.7. Capability discovery tools
| Tool | Описание |
|---|---|
| `discover_project_capabilities` | Просканировать приложение и найти все доступные возможности (routes, publish bindings, UI actions, функции). Если указан project_id — только для этого проекта. |
| `list_project_actions` | Показать нормализованный список действий для проекта, сгруппированных по intent (publish, enqueue, launch, retry, delete, edit, schedule) |
| `invoke_project_action` | Попытаться выполнить найденное действие. Для endpoint — HTTP-вызов, для ui_form — эмуляция POST. Для destructive действий требуется подтверждение. |
| `refresh_capability_registry` | Форсировать полный рескан capability registry — пересканировать routes, проекты, UI actions, код. |
| `fetch_json_url` | Загрузить JSON по URL с пониманием content-type. Для openapi.json, json-конфигов, API-ответов. |

### 5.8. Navigation tools
| Tool | Описание |
|---|---|
| `navigate_to` | Перенаправить браузер пользователя на указанную страницу. Используется после запуска пайплайна (→ страница run), после создания расписания (→ страница проекта), и т.д. |
| `reload_page` | Обновить текущую страницу. Используется после apply_file_patch, update_config — чтобы изменения отобразились сразу. |
...
| CapabilityRegistry | capabilities.json | Кэш обнаруженных возможностей приложения (routes, actions, UI forms, bindings) |
...
## 10. Dynamic capability discovery

### 10.1. Назначение

Маршал умеет не только пользоваться заранее описанными tools, но и динамически обнаруживать новые возможности в приложении Marshrutka.

Когда пользователь просит операционное действие (publish, enqueue, launch, retry, delete, schedule, edit), агент:
1. Пробует прямые tools
2. Если прямого tool нет — вызывает `discover_project_capabilities` для сканирования
3. Проверяет найденные actions через `list_project_actions`
4. При наличии подходящего endpoint — пытается выполнить через `invoke_project_action`
5. Только если action не найден — объясняет конкретно, чего не хватает

### 10.2. Что сканируется

**Backend routes** — AST-парсинг `api.py`, извлекаются все FastAPI-маршруты с методом, path, handler. Классифицируются по intents (publish, enqueue, launch, schedule, delete, edit).

**Project configs** — читаются `projects/*.json`, извлекаются publish_bindings, integration (api_url), primary_artifact, defaults. publish_bindings преобразуются в config_binding actions.

**UI actions** — сканируются `.html` шаблоны на form actions, POST-кнопки, ссылки с method=POST. Всё, что доступно пользователю через UI, считается potential action.

**Code patterns** — AST-поиск Python-функций по именам (publish_*, enqueue_*, launch_*, delete_*, retry_*).

### 10.3. CapabilityRegistry

`agent/capabilities.py` — центральный модуль.

**CapabilityRegistry:**
- `scan_app()` — полное сканирование, возвращает structured registry
- `refresh()` — scan + save кэша
- `load()` / `save()` — загрузка/сохранение кэша `data/agent/capabilities.json`
- `get_project_actions(project_id)` — actions для конкретного проекта
- `find_actions_for_intent(intent, project_id?)` — поиск actions по intent
- `match_intent(user_message)` — определение intent из сообщения (русские + английские ключевые слова)

**OperationalIntentAnalyzer:**
- Определяет, является ли запрос operational
- Извлекает project_id из текста сообщения
- Находит matching actions в registry

### 10.4. Нормализованный реестр actions

```json
{
  "action_id": "route.api.projects.{project_id}.queue.{queue_item_id}.launch.post",
  "project_id": "{project_id}",
  "label": "Launch Queue Item",
  "kind": "endpoint",
  "method": "POST",
  "path": "/api/projects/{project_id}/queue/{queue_item_id}/launch",
  "source": "route",
...
Поддерживаемые `kind`:
- `endpoint` — HTTP-ручка, исполнима через invoke_project_action
- `ui_form` — HTML-форма/кнопка, исполнима через эмуляцию POST
- `internal_function` — Python-функция (unsafe, только manual)
- `config_binding` — publish binding из конфига проекта

### 10.5. Response quality (anti-refusal)

Если агент исследовал проект и нашёл признаки capabilities, он НЕ ДОЛЖЕН отвечать generic refusal.

Вместо "у меня нет доступа к вашим данным" ответ должен содержать:
- что уже найдено (runs, код, конфиг),
- какие app capabilities обнаружены,
- что удалось выполнить,
- чего не хватило,
- что можно сделать дальше (вызвать action / сделать patch / manual call).

Это контролируется функцией `_enhance_response_if_needed()` в `harness.py`.

### 10.6. Capability summary в system prompt

При сборке system prompt в `build_system_prompt()` добавляется краткая сводка из capability registry: какие actions доступны для каждого проекта. Это даёт модели контекст о возможностях приложения без полного сканирования на каждом шаге.
...
1. Написать функцию в соответствующем файле (`tools.py`, `file_tools.py`, `research.py`, `operator_tools.py`, `capabilities.py`)
...
- **Capability discovery сканирует только Python/HTML файлы** — не проверяет runtime-состояние приложения (запущенные процессы, динамически загруженные модули).
- **invoke_project_action для internal_function небезопасен** — автоматический вызов произвольной функции запрещён, только manual path.
- **Capability registry не обновляется автоматически** — требуется явный вызов `refresh_capability_registry` или `discover_project_capabilities`. При старте сервера registry загружается из кэша, а не сканируется заново.
- **UI actions извлекаются из шаблонов, а не из DOM** — если форма динамически добавляется через JavaScript, она не будет обнаружена.
